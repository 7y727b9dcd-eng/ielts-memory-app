...
这个目录把根目录的“聆听训练”PWA 包装成 iPhone/iPad 原生容器，并通过 GitHub Actions 生成供 AltStore 安装的未签名 IPA。
...
5. 点击 `Run workflow`。
6. 构建完成后，在运行记录底部下载 `ListeningTraining-AltStore` artifact。
7. 解压 artifact，得到 `ListeningTraining-AltStore.ipa`。
...
3. 在设备上打开 AltStore。
4. 进入 `My Apps`，点击左上角 `+`。
5. 从“文件”中选择 `ListeningTraining-AltStore.ipa`。
